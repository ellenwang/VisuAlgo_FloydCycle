...
var List = function(){
  ...
  this.generateRandomFixedSize = function(vertexText) {
      if(activeStatus=="stack"){
        if(vertexText>maxStackSize){
           $('#create-err').html("Sorry, maximum allowed number of vertex is " + maxStackSize);
           return false;
        }
      }
      else{
        if(vertexText>maxSize){
           $('#create-err').html("Sorry, maximum allowed number of vertex is " + maxSize);
           return false;
        }
      }
  ...
  this.generateUserDefined = function(vertexTextArr){
      ...
      if (vertexTextArr == '') { // prevent creation of empty list
        $('#create-err').html("Sorry, minimum number of vertex is at least 1");
        return false;
      }
...
this.removeArrTailDoublyList = function(){
  ...
  this.removeArrKth = function(vertexTextArr){
    ...
    currentVertexClass = internalList[currentVertex]["vertexClassNumber"];
    currentState["vl"][currentVertexClass]["state"] = VERTEX_HIGHLIGHTED;
    currentState["status"] = "cur points to head";
    currentState["lineNo"] = 1;
    stateList.push(currentState);
    //end
      ...
      // while(true){
        ...
        currentState["el"][edgeHighlighted]["animateHighlighted"] = true;
        currentState["el"][edgeHighlighted]["state"] = EDGE_TRAVERSED;
        currentState["status"] = "cur points to the next vertex";
        currentState["lineNo"] = 3;
        stateList.push(currentState);
        //end
      }
      ...
      currentState["vl"][currentVertexClass]["state"] = VERTEX_HIGHLIGHTED;
      vertexTraversed[currentVertex] = true;
      currentState["status"] = "k is 0, cur now points to one vertex behind the vertex to-be-deleted. We stop searching and continue with the removal";
      currentState["lineNo"] = 2;
      stateList.push(currentState); 
      //end
      ...
      if(currentVertex != null){
        ...
        vertexTraversed[currentVertex] = true;
        currentState["status"] = "We store reference to the vertex to-be-deleted";
        currentState["lineNo"] = 4;
        stateList.push(currentState);
        //end
      }
      ...
      if(internalList[currentVertex]["rightChild"]==null){
        ...
        currentState["status"] = "We connect the vertex behind the vertex to-be-deleted with the next vertex after the vertex to-be-deleted";
        currentState["lineNo"] = 5;
        stateList.push(currentState);
        //end
        ...
        currentState["status"] = "Now we delete this vertex";
        currentState["lineNo"] = 6;
        stateList.push(currentState);
        //end
  ...
  this.removeArrKthDoublyList = function(vertexTextArr){
    ...
    // Find vertex
      ...
      if(currentVertex != null){
        ...
        vertexTraversed[currentVertex] = true;
        currentState["status"] = "keep track of the vertex after the vertex to-be-deleted";
        currentState["lineNo"] = 5;
        stateList.push(currentState);
        //end
      }
  ...
  function populatePseudocode(act) {
    ...
    case 6: // remove TAIL
        ...
        break;
    case 7: // remove kth
        document.getElementById('code1').innerHTML = 'Vertex cur = head';
        document.getElementById('code2').innerHTML = 'while (--k!=0)';
        document.getElementById('code3').innerHTML = '&nbsp&nbspcur = cur.next';
        document.getElementById('code4').innerHTML = 'Vertex tobedeleted = cur.next';
        if((activeStatus == "doublylist")||(activeStatus == "deque")){
          document.getElementById('code5').innerHTML = 'Vertex temp = tobedeleted.next';
          document.getElementById('code6').innerHTML = 'delete tobedeleted';
          document.getElementById('code7').innerHTML = 'cur.next = temp, temp.prev = cur';
        }
        else{
          document.getElementById('code5').innerHTML = 'cur.next = cur.next.next';
          document.getElementById('code6').innerHTML = 'delete tobedeleted';
          document.getElementById('code7').innerHTML = '';
        }
        break;
