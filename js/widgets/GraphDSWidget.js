function GraphVisu(arg1, arg2, arg3)
{
	...
	console.log(json);
		...
		for (var j = 0; j < nodes.length; j++) tempVisited[j] = false;
	
		dfsTraverse(nodes[0].id);

		function dfsTraverse(t)
		{
		    tempVisited[t] = true;

			for (var k = 0; k < AdjList[t].length; k++)
				if (tempVisited[AdjList[t][k]] === false) 
					dfsTraverse(AdjList[t][k]);
		}

		for (var i = 0; i < nodes.length; i++)
		{
			if (tempVisited[i] === false) {
				console.log(tempVisited[i]);
				CONNECTED = false;
				break;
			}
